...
switch ($method) {
    case 'POST':
        ...
        if (isset($_GET['action']) && $_GET['action'] == 'upload') {
            ...
            $jamiathulHindId = secure_input($conn, $input, 'jamiathulHindId');
            $campus = secure_input($conn, $input, 'campus');
            $jamiaClass = secure_input($conn, $input, 'jamiaClass');
            $academicClass = secure_input($conn, $input, 'academicClass');
            $academicCourse = secure_input($conn, $input, 'academicCourse');
            $batch = secure_input($conn, $input, 'batch');
            $status = secure_input($conn, $input, 'status');
            $email = secure_input($conn, $input, 'email');
            $phone1 = secure_input($conn, $input, 'phone1');
            $phone2 = secure_input($conn, $input, 'phone2'); // Optional field
            ...
            $admissionYear = intval($input['admissionYear']);
            $image = secure_input($conn, $input, 'image'); // Handle image as a string for now

            // Handle optional fields - set as empty string if not provided or provided as empty
            $prismMember = secure_input($conn, $input, 'prismMember');
            $foundationCourse = secure_input($conn, $input, 'foundationCourse');
            $foundationCampus = secure_input($conn, $input, 'foundationCampus');
            $bachelorCourse = secure_input($conn, $input, 'bachelorCourse');
            $bachelorCampus = secure_input($conn, $input, 'bachelorCampus');
            $finishingCourse = secure_input($conn, $input, 'finishingCourse');
            $finishingCampus = secure_input($conn, $input, 'finishingCampus');
            $pgCourse = secure_input($conn, $input, 'pgCourse');
            $pgCampus = secure_input($conn, $input, 'pgCampus');
            $passport = secure_input($conn, $input, 'passport');
            $rehlaReport = secure_input($conn, $input, 'rehlaReport');
            $internship = secure_input($conn, $input, 'internship');
            $dissertation = secure_input($conn, $input, 'dissertation');

            // SQL query
            $sql = "INSERT INTO students (
                    std_fullname, std_place, std_jamia_id, std_jamiathulhind_id, std_image, std_cur_campus, 
                    std_jamia_class, std_academic_class, std_academic_course, std_batch_no, std_prism_member, 
                    std_status, std_email, std_phone1, std_phone2, std_father, std_mother, std_dob, std_address, 
                    std_taluk, std_district, std_state, std_pincode, std_aadhar, std_passport, std_admission_year, foundation_course, 
                    foundation_campus, bachelor_course, bachelor_campus, finishing_course, finishing_campus, pg_course, 
                    pg_campus, rehla_report, internship, dissertation
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
